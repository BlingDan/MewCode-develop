...
public final class AnthropicClient implements LlmClient {
  ...
  private void streamInCurrentThread(
    ...
    try {
      ...
      if (thinking) {
        params.thinking(
            ThinkingConfigEnabled.builder().budgetTokens(THINKING_BUDGET_TOKENS).build());
      }
        ...
        while (iterator.hasNext()) {
          ...
          if (event.isMessageStart()) {
            ...
            if (delta.isText()) {
              ...
              putEvent(queue, new StreamEvent.ThinkingDelta("", delta.asSignature().signature()));
  ...
  private static List<MessageParam> buildMessages(List<Message> messages) {
    ...
    for (Message message : messages) {
      ...
      MessageParam.Role role =
          "assistant".equals(message.role()) ? MessageParam.Role.ASSISTANT : MessageParam.Role.USER;
