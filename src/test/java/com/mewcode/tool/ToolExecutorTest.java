...
class ToolExecutorTest {
  ...
  void validatesBeforeExecutionAndReturnsStructuredError() {
    ...
    registry.register(new TestTool("Validate", true, calls, null, 0, "参数不合法，请调整后重试。"));

    try (var executor = new ToolExecutor(registry, context())) {
      ToolInvocationResult result =
          executor.executeSingle(new ToolCall("call-1", "Validate", Map.of()));
  ...
  void validatesRelativePathsAgainstTheSharedRootBeforeAnyToolRuns() {
    ...
    List<ToolCall> calls =
        List.of(
            ...
            new ToolCall(
                "edit",
                "EditFile",
                Map.of("path", "README.md", "old_string", "old", "new_string", "new")),
    ...
    try (var executor = new ToolExecutor(registry, context())) {
      for (ToolCall call : calls) {
        ...
        assertTrue(
            result.result().content().contains(tempDir.toAbsolutePath().normalize().toString()),
  ...
  void runsSafeCallsConcurrentlyAndPreservesInputOrder() throws Exception {
    ...
    var registry = new ToolRegistry();
    registry.register(new TestTool("Safe", true, new AtomicInteger(), started, 0, null, release));

    try (var executor = new ToolExecutor(registry, context())) {
      try (var waiter = Executors.newVirtualThreadPerTaskExecutor()) {
        Future<List<ToolInvocationResult>> future =
            waiter.submit(
                () ->
  ...
  void serialToolsNeverOverlap() {
    ...
    try (var executor = new ToolExecutor(registry, context())) {
      ...
      assertEquals(
          List.of("one", "two"), results.stream().map(ToolInvocationResult::toolUseId).toList());
  ...
  void cancellationReturnsWithoutWaitingForToolTimeout() throws Exception {
    ...
    var registry = new ToolRegistry();
    registry.register(new TestTool("Blocking", true, new AtomicInteger(), started, 10_000, null));
    var token = new CancellationToken();
  ...
  @Test
  void batchRunsPreAndPostHooksOnceInInputOrder() {
    var executions = new AtomicInteger();
    var registry = new ToolRegistry();
    registry.register(new TestTool("Safe", true, executions, null, 0, null));
    var rules =
        List.of(
            new HookRule(
                "pre",
    ...
    try (var hooks =
            new HookEngine(
                ...
                new CommandRunner(new HookSandbox()),
                ignored -> {});
        var executor = new ToolExecutor(registry, context())) {
      executor.configureHooks(hooks, state);
      List<ToolInvocationResult> results =
          executor.executeBatch(
              List.of(
                  new ToolCall("one", "Safe", Map.of()),
                  new ToolCall("two", "Safe", Map.of()),
                  new ToolCall("same", "Safe", Map.of()),
                  new ToolCall("same", "Safe", Map.of())));
      assertEquals(
          List.of("one", "two", "same", "same"),
          results.stream().map(ToolInvocationResult::toolUseId).toList());
      assertEquals(3, executions.get());
      assertEquals(
          List.of("pre", "pre", "pre", "pre", "post", "post", "post", "post"),
          state.snapshotPrompts().texts());
    }
  }
  ...
  private static final class HookSandbox implements BashSandbox {
    @Override
