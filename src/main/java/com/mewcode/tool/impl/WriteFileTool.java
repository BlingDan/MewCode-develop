    ...
    try {
      ...
      if (existing) {
        if (TextFileSupport.isBinary(path)) {
          return ToolResult.error("拒绝覆盖二进制文件：" + path + "。请使用 Bash 处理二进制内容。");
        }
        if (!context.fileStateCache().wasRead(path)) {
          return ToolResult.error("拒绝覆盖未读取的已有文件：" + path + "。请先调用 ReadFile，确认内容后再重试。");
        }
        if (!context.fileStateCache().canModify(path)) {
          return ToolResult.error("文件在读取后已发生变化：" + path + "。为避免覆盖外部修改，请先重新调用 ReadFile 后再重试。");
        }
      }
      ...
      if (existing) {
        Files.writeString(
            path, content, StandardOpenOption.TRUNCATE_EXISTING, StandardOpenOption.WRITE);
      } else {
      ...
      return ToolResult.error("文件在写入过程中已被创建：" + path + "。请重新读取文件状态后再重试。");
