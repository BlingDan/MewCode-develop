 ...
 *
 * <p>事件按产生顺序进入单次运行的流：文本可以增量展示，工具结果一次性展示， {@link LoopComplete} 是本次交互的收口标记。取消不会伪装成错误事件，UI 由取消路径
 * 自己展示“已取消”并回到空闲态。
 */
public sealed interface AgentEvent
  ...
  record ToolUse(String requestId, String toolName, Map<String, Object> input)
  ...
  record ToolResult(
      String requestId, String toolName, String result, boolean isError, long durationMillis)
      implements AgentEvent {
    public ToolResult {
      requestId = requireText(requestId, "requestId");
      toolName = requireText(toolName, "toolName");
  ...
  record Usage(OptionalLong inputTokens, OptionalLong outputTokens) implements AgentEvent {
    public Usage {
