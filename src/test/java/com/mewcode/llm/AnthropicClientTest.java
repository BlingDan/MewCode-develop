...
class AnthropicClientTest {
  ...
  void streamsThinkingTextAndCompleteInOrder() throws Exception {
    ...
    try {
      ...
      String projectRoot =
          Path.of("/tmp/mewcode-anthropic-project").toAbsolutePath().normalize().toString();
  ...
  @Test
  void serializesStructuredSystemSegmentsAndAppendsReminderToLastUserMessage() throws Exception {
    String fixture = resource("/sse/anthropic-tool-use.txt");
    var body = new AtomicReference<String>();
    HttpServer server =
        server(
  ...
  void authenticationErrorIsSafeAndNotRetried() throws Exception {
    ...
    try {
      ...
      assertFalse(
          ((StreamEvent.Error) events.getFirst()).message().contains("unique-anthropic-key"));
