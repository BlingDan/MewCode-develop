...
class ToolPolicyTest {
  ...
  @Test
  void subagentPolicyUsesAnAbsoluteIntersectionAndBlocksTaskRecursion() {
    var registry = ToolRegistry.createDefault();
    registry.register(new AgentTool());
    try (var tasks = new SubAgentTaskManager()) {
      TaskTools.registerAll(registry, tasks, () -> "session");
      var spec =
          new SubAgentSpec(
              "reader",
              "read",
              Set.of("ReadFile", "Bash"),
              Set.of("Bash"),
              "read",
              3,
              "inherit",
              SubAgentSpec.PermissionMode.DEFAULT,
              SubAgentSpec.Source.PROJECT,
              Path.of("/tmp/reader.md"));
      var policy =
          ToolPolicy.forSubAgent(registry, ToolPolicy.forMode(AgentMode.EXECUTE), spec, false);

      assertTrue(policy.isAllowed(registry.get("ReadFile").orElseThrow()));
      assertFalse(policy.isAllowed(registry.get("Bash").orElseThrow()));
      assertFalse(policy.isAllowed(registry.get("Agent").orElseThrow()));
      assertFalse(policy.isAllowed(registry.get("TaskList").orElseThrow()));
    }
  }
