...
public final class GrepTool implements Tool {
    ...
    public ToolResult execute(ToolExecutionContext context, Map<String, Object> input) {
        ...
        PathMatcher include = input.get("include") instanceof String value && !value.isBlank()
        ...
        try {
            ...
            return ToolResult.error("Grep 搜索失败：" + error.getMessage()
                    + "。请检查搜索根目录权限后重试。");
        }

        groups.sort(Comparator.comparing((MatchGroup group) -> SearchSupport.modifiedTime(group.file())).reversed()
