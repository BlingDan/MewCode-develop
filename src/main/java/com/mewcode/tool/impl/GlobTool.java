...
public final class GlobTool implements Tool {
    ...
    public Map<String, Object> inputSchema() {
        return Map.of(
                "type", "object",
                "properties", Map.of(
                        "pattern", Map.of("type", "string", "description", "项目根目录内的绝对 glob 模式")),
    ...
    public String validateInput(ToolExecutionContext context, Map<String, Object> input) {
        String patternError = ToolInput.requireString(
                input, "pattern", " 请传入例如 /项目根目录/**/*.java 的绝对模式。");
    ...
    public ToolResult execute(ToolExecutionContext context, Map<String, Object> input) {
        ...
        String patternError = PathGuard.validatePattern(patternText, context.projectRoot());
        ...
        try {
            ...
            String rootError = PathGuard.validatePath(searchRoot.toString(), context.projectRoot(), true);
            if (rootError != null) return ToolResult.error("glob 搜索根目录不可用：" + rootError);
            if (!Files.isDirectory(searchRoot)) {
                return ToolResult.error("glob 搜索根目录不存在或不是目录：" + searchRoot
                        + "。请调整绝对模式后重试。");
            }
            ...
            return ToolResult.success(matches.stream()
                    .limit(SearchSupport.MAX_RESULTS)
                    .map(path -> SearchSupport.relative(searchRoot, path))
                    .reduce((left, right) -> left + "\n" + right)
                    .orElse(""));
        } catch (RuntimeException | IOException error) {
            return ToolResult.error("glob 搜索失败：" + error.getMessage()
                    + "。请检查模式和搜索根目录后重试。");
        }
    }
