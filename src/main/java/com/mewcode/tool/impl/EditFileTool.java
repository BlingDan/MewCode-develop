...
public final class EditFileTool implements Tool {
    ...
    public ToolResult execute(ToolExecutionContext context, Map<String, Object> input) {
        ...
        try {
            ...
            if (occurrences == 0) {
                return ToolResult.error("未找到 old_string，文件未修改：" + path
                        + "。请先 ReadFile 核对原文和空白字符。");
            }
            if (occurrences > 1) {
                return ToolResult.error("old_string 在文件中出现多次，替换不唯一，文件未修改：" + path
                        + "。请扩大匹配文本使其只出现一次。");
            }
